...
def _preprocess_data(args):
    ...
    all = {
        "path": train_list_wavs + test_list_wavs,
        # "wav_filesize": train_list_size + test_list_size,
        "transcript": train_list_trans + test_list_trans,
    }

    df_all = pd.DataFrame(
        all, columns=["path", "transcript"], dtype=int
    )
    df_train = pd.DataFrame(
        a, columns=["path", "transcript"], dtype=int
    )
    df_test = pd.DataFrame(
        c, columns=["path", "transcript"], dtype=int
    )

    df_all.to_csv(
        target + "/timit_all.csv", sep=",", header=True, index=False, encoding="utf-8"
    )
    df_train.to_csv(
        target + "/timit_train.csv", sep=",", header=True, index=False, encoding="utf-8"
    )
    df_test.to_csv(
        target + "/timit_test.csv", sep=",", header=True, index=False, encoding="utf-8"
    )
